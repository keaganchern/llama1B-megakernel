...
def _find_hlo_file(submodel_dir):
    # New SDK names files like model.MODULE_<hash>.hlo_module.pb; old SDK used model/graph.hlo.
    import glob
    for pattern in (
        f"{submodel_dir}/model/graph.hlo",
        f"{submodel_dir}/model.MODULE_*.hlo_module.pb",
        f"{submodel_dir}/*.hlo_module.pb",
    ):
...
def count_nki_flop_ratio(
    hlo_path_context_enc=None,
    hlo_path_token_gen=None,
):
    if hlo_path_context_enc is None:
        hlo_path_context_enc = _find_hlo_file("/tmp/nxd_model/context_encoding_model/_tp0_bk0")
    if hlo_path_token_gen is None:
        hlo_path_token_gen = _find_hlo_file("/tmp/nxd_model/token_generation_model/_tp0_bk0")

    if hlo_path_context_enc is None or hlo_path_token_gen is None:
        print(f"WARN: HLO files not found (context={hlo_path_context_enc}, token={hlo_path_token_gen}); returning 0 NKI ratio")
        return 0.0
...
def main():
    ...
    # --enable-nki selects the NKI-enabled model class (llama_with_megakernel)
    # over the XLA baseline (llama). --llama still works as an override if set.
    if args.llama == "llama" and args.enable_nki:
        llama_module_name = "llama_with_megakernel"
    else:
